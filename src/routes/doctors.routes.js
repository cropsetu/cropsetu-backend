...
router.get('/nearby', async (req, res) => {
  ...
  const withDist = doctors
    .map(d => {
      ...
      const distKm = haversineKm(latF, lngF, d.latitude, d.longitude);
...
router.post('/:id/review', authenticate, async (req, res) => {
  ...
  const safeComment = comment ? comment.replace(/<[^>]*>/g, '').slice(0, 500) : null;
  ...
  const review = existing
    ? await prisma.doctorReview.update({
        ...
        data:  { rating: parsedRating, comment: safeComment },
      })
