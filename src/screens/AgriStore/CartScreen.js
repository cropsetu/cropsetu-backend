...
function ShimmerBox({ style }) {
  ...
  return (
    <View style={[{ backgroundColor: '#E8EBF0', overflow: 'hidden', borderRadius: 8 }, style]}>
    ...
    <SafeAreaView style={S.container}>
      <Animated.View style={[S.emptyWrap, { opacity: op, transform: [{ translateY: fadeY }] }]}>
        <Animated.View style={[S.emptyIconWrap, { transform: [{ scale: pulse }] }]}>
          <Ionicons name="bag-outline" size={56} color={GREEN} />
        </Animated.View>
        <Text style={S.emptyTitle}>Your cart is empty</Text>
        <Text style={S.emptySub}>Add some farming essentials to get started!</Text>
        <PressScale onPress={() => navigation.goBack()} down={0.96} style={S.shopBtnWrap}>
          <LinearGradient colors={['#48BB78', GREEN]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={S.shopBtnGrad}>
            <Ionicons name="storefront-outline" size={18} color="#fff" />
            <Text style={S.shopBtnTxt}>Continue Shopping</Text>
          </LinearGradient>
        </PressScale>
      </Animated.View>
    </SafeAreaView>
...
// ── Cart Item ─────────────────────────────────────────────────────────────────
function CartItem({ item, onQtyChange, onRemove, index }) {
  const op    = useRef(new Animated.Value(0)).current;
  ...
  function confirmRemove() {
    Alert.alert('Remove Item', `Remove "${product.name}" from your cart?`, [
  ...
  return (
    <View style={S.progressWrap}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        <Ionicons name="car-outline" size={15} color={GREEN} />
        <Text style={S.progressTxt}>
          Add <Text style={{ color: GREEN, fontWeight: '700' }}>₹{(threshold - current).toLocaleString()}</Text> more for free delivery!
        </Text>
      </View>
  ...
  return (
    <SafeAreaView style={S.container} edges={['top', 'left', 'right']}>
      ...
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} tintColor={GREEN} colors={[GREEN]} />}
        renderItem={({ item, index }) => (
          <CartItem item={item} onQtyChange={handleQtyChange} onRemove={handleRemove} index={index} />
        )}
        ListFooterComponent={(
          <View style={S.summaryCard}>
            ...
            <View style={S.summaryRow}>
              ...
              <Text style={[S.summaryValue, delivery === 0 && { color: GREEN, fontWeight: '700' }]}>
...
const S = StyleSheet.create({
  ...
  headerBack:    { width: 38, height: 38, borderRadius: 19, backgroundColor: '#F5F5F5', justifyContent: 'center', alignItems: 'center' },
  headerTitle:   { fontSize: 16, fontWeight: '800', color: '#1A1A1A' },
  headerSub:     { fontSize: 12, color: MUTED, marginTop: 1 },
  headerBadge:   { width: 32, height: 32, borderRadius: 16, backgroundColor: GREEN, justifyContent: 'center', alignItems: 'center' },
  headerBadgeTxt:{ color: '#fff', fontSize: 13, fontWeight: '800' },
  ...
  itemImgBox: { width: 80, height: 80, borderRadius: 12, backgroundColor: '#F0F4F0', overflow: 'hidden', justifyContent: 'center', alignItems: 'center' },
  itemImg:    { width: '100%', height: '100%' },
  itemCat:    { fontSize: 10, color: GREEN, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.8 },
  itemName:   { fontSize: 14, fontWeight: '700', color: '#1A1A1A', marginTop: 2, lineHeight: 19 },
  itemPrice:  { fontSize: 14, fontWeight: '800', color: GREEN, marginTop: 4 },
  itemUnit:   { fontSize: 12, fontWeight: '400', color: MUTED },
  trashBtn:   { width: 36, height: 36, borderRadius: 18, backgroundColor: '#FEE2E2', justifyContent: 'center', alignItems: 'center' },

  itemCardFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, paddingTop: 14, borderTopWidth: 1, borderTopColor: BORDER },

  // Pill quantity selector
  qtyPill:    { flexDirection: 'row', alignItems: 'center', gap: 0, backgroundColor: '#F1F3F4', borderRadius: 50, padding: 4 },
  qPillBtn:   { width: 34, height: 34, borderRadius: 17, backgroundColor: CARD, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 3, elevation: 1 },
  qNum:       { fontSize: 15, fontWeight: '800', color: '#1A1A1A', minWidth: 32, textAlign: 'center' },
  itemSubtotal: { fontSize: 17, fontWeight: '800', color: '#1A1A1A' },
  ...
  summaryTitle: { fontSize: 15, fontWeight: '800', color: '#1A1A1A', marginBottom: 16 },
  summaryRow:   { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  summaryLabel: { fontSize: 14, color: MUTED },
  summaryValue: { fontSize: 14, fontWeight: '600', color: '#1A1A1A' },
  summaryDivider:{ borderTopWidth: 1, borderTopColor: BORDER, marginVertical: 10 },
  totalLabel:   { fontSize: 15, fontWeight: '800', color: '#1A1A1A' },
  totalValue:   { fontSize: 19, fontWeight: '900', color: GREEN },
  savingsBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 8 },
  savingsTxt:   { fontSize: 12, color: GREEN, fontWeight: '600' },

  // Free delivery progress
  progressWrap: { paddingVertical: 10 },
  progressTxt:  { fontSize: 13, color: MUTED, flex: 1 },
  progressTrack:{ height: 7, backgroundColor: '#E8EAED', borderRadius: 10, overflow: 'hidden' },
  ...
  barTotal:     { fontSize: 17, fontWeight: '900', color: GREEN },
  barSub:       { fontSize: 10, color: MUTED, marginTop: 1 },
  checkoutBtn:  { borderRadius: 14, overflow: 'hidden' },
  checkoutGrad: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 22, paddingVertical: 14, backgroundColor: GREEN },
