...
const R = StyleSheet.create({
  ...
  avatar:     { width: 36, height: 36, borderRadius: 18, backgroundColor: '#E8F5E9',
                alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 15, fontWeight: '700', color: '#1A5C2A' },
  body:       { flex: 1 },
  top:        { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 2 },
  name:       { fontSize: 13, fontWeight: '700', color: '#222' },
  ago:        { fontSize: 11, color: '#999' },
  stars:      { fontSize: 14, color: '#F59E0B', marginBottom: 3 },
  ...
  if (loading) {
    return (
      <View style={[S.center, { paddingTop: insets.top }]}>
        ...
        <Text style={{ color: '#666', marginTop: 10 }}>{t('doctor.loading')}</Text>
      </View>
    );
  }
    ...
    return (
      <View style={[S.center, { paddingTop: insets.top }]}>
        ...
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Text style={{ color: '#1A5C2A', marginTop: 8 }}>{t('doctor.goBack')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={[S.root, { paddingTop: insets.top }]}>
      ...
      <TouchableOpacity style={S.backBtn} onPress={() => navigation.goBack()}>
        <Ionicons name="arrow-back" size={22} color="#1A5C2A" />
      </TouchableOpacity>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={S.scroll}>
        ...
        <View style={S.hero}>
          <View style={S.heroAvatar}>
            <Ionicons name="person" size={44} color="#1A5C2A" />
          </View>
          ...
          <View style={S.heroMeta}>
            {available ? (
              ...
              <View style={[S.availPill, { backgroundColor: '#FEF3C7' }]}>
                <Text style={[S.availPillText, { color: '#92400E' }]}>{t('doctor.todayClosed')}</Text>
              </View>
            )}
            {location ? (
        ...
        <View style={S.section}>
          ...
          {quals.map((q, i) => (
            <View key={i} style={S.qualRow}>
              <Ionicons name="ribbon-outline" size={14} color="#1A5C2A" />
          ...
          {doctor.experienceYears > 0 && (
            <View style={S.qualRow}>
              <Ionicons name="time-outline" size={14} color="#1A5C2A" />
              <Text style={S.qualText}>{t('doctor.experienceYears', { n: doctor.experienceYears })}</Text>
            </View>
          )}
          {doctor.clinicName && (
            <View style={S.qualRow}>
              <Ionicons name="business-outline" size={14} color="#1A5C2A" />
              <Text style={S.qualText}>{doctor.clinicName}</Text>
            </View>
          )}
          {doctor.registrationNumber && (
            <View style={S.qualRow}>
              <Ionicons name="card-outline" size={14} color="#1A5C2A" />
            ...
            <View style={S.tagWrap}>
              {doctor.animalLabels.map((a, i) => (
                <View key={i} style={S.tag}>
                  <Text style={S.tagText}>{t(`doctor.animals.${a.key}`) || a.mr || a.en}</Text>
                </View>
              ))}
            </View>
          </View>
            ...
            <View style={S.serviceGrid}>
              {doctor.serviceLabels.map((sv, i) => (
                <View key={i} style={S.serviceItem}>
                  <Ionicons name="checkmark-circle" size={14} color="#1A5C2A" />
                  <Text style={S.serviceText}>{t(`doctor.services.${sv.key}`) || sv.mr || sv.en}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View style={S.section}>
          ...
          {loadingRev && <ActivityIndicator style={{ margin: 12 }} color="#1A5C2A" />}
...
const S = StyleSheet.create({
  ...
  // Hero
  hero:         { backgroundColor: '#FFF', alignItems: 'center', paddingTop: 56, paddingBottom: 20,
                  paddingHorizontal: 20, marginBottom: 12,
                  shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
                  shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  heroAvatar:   { width: 80, height: 80, borderRadius: 40, backgroundColor: '#E8F5E9',
                  alignItems: 'center', justifyContent: 'center', borderWidth: 3,
                  borderColor: '#1A5C2A', marginBottom: 12 },
  heroName:     { fontSize: 20, fontWeight: '900', color: '#1A1A1A', textAlign: 'center', marginBottom: 6 },
  heroRatingRow:{ flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 10 },
  heroRating:   { fontSize: 16, fontWeight: '800', color: '#1A1A1A' },
  heroRatingCount: { fontSize: 12, color: '#888' },
  ...
  availPillText:{ fontSize: 12, fontWeight: '700', color: '#15803D' },
  ...
  // Section
  section:      { backgroundColor: '#FFF', marginHorizontal: 12, marginBottom: 10,
                  borderRadius: 14, padding: 16,
                  shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
                  shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  sectionTitle: { fontSize: 15, fontWeight: '800', color: '#1A5C2A', marginBottom: 10 },

  // Qualifications
  qualRow:      { flexDirection: 'row', gap: 8, marginBottom: 7, alignItems: 'flex-start' },
  qualText:     { flex: 1, fontSize: 13, color: '#444', lineHeight: 19 },

  // Animal tags
  tagWrap:      { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  tag:          { backgroundColor: '#E8F5E9', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 },
  tagText:      { fontSize: 13, color: '#2D6A4F', fontWeight: '600' },

  // Services
  serviceGrid:  { gap: 8 },
  serviceItem:  { flexDirection: 'row', alignItems: 'center', gap: 8 },
  serviceText:  { fontSize: 13, color: '#444' },

  // Availability
  availInfo:    { fontSize: 13, color: '#444', lineHeight: 20, marginBottom: 8 },
  emergencyBadge: { flexDirection: 'row', alignItems: 'center', gap: 6,
                    backgroundColor: '#FEE2E2', paddingHorizontal: 10, paddingVertical: 7,
                    borderRadius: 8 },
  emergencyText:  { fontSize: 13, color: '#DC2626', fontWeight: '700' },

  // Fees
  feeRow:       { flexDirection: 'row', gap: 12 },
  feeCard:      { flex: 1, backgroundColor: '#F0F7F0', borderRadius: 10, padding: 12, alignItems: 'center' },
  feeLabel:     { fontSize: 11, color: '#666', marginBottom: 4 },
  feeAmt:       { fontSize: 20, fontWeight: '900', color: '#1A5C2A' },
  feeNote:      { fontSize: 12, color: '#888', marginTop: 8 },

  // Languages
  langText:     { fontSize: 13, color: '#444', textTransform: 'capitalize' },

  // Reviews
  revHeader:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  writeRevBtn:  { backgroundColor: '#E8F5E9', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  writeRevBtnText: { fontSize: 13, color: '#1A5C2A', fontWeight: '700' },
  reviewForm:   { backgroundColor: '#F9FFF9', borderRadius: 10, padding: 14, marginBottom: 14,
                  borderWidth: 1, borderColor: '#C8E6C9', gap: 12 },
  revFormTitle: { fontSize: 14, fontWeight: '700', color: '#1A5C2A' },
  commentInput: { borderWidth: 1, borderColor: '#DDD', borderRadius: 8, padding: 10,
                  fontSize: 13, color: '#333', minHeight: 70, textAlignVertical: 'top',
                  backgroundColor: '#FFF' },
  submitRevBtn: { backgroundColor: '#1A5C2A', borderRadius: 10, paddingVertical: 12,
                  alignItems: 'center' },
  submitRevBtnText: { color: '#FFF', fontSize: 15, fontWeight: '800' },
  loadMoreBtn:  { paddingVertical: 12, alignItems: 'center' },
  loadMoreText: { color: '#1A5C2A', fontSize: 13, fontWeight: '700' },
  noReview:     { color: '#999', fontSize: 13, textAlign: 'center', paddingVertical: 16 },
  ...
  stickyCall:   { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
                  gap: 6, backgroundColor: '#1A5C2A', borderRadius: 12, paddingVertical: 13 },
  stickyCallText: { color: '#FFF', fontSize: 15, fontWeight: '800' },
  stickyWa:     { flex: 1.3, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
                  gap: 6, backgroundColor: '#25D366', borderRadius: 12, paddingVertical: 13 },
  stickyWaText: { color: '#FFF', fontSize: 15, fontWeight: '800' },
});
