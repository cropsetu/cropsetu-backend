...
function VoiceSphere({ isListening, audioLevel }) {
  ...
  return (
    <View style={{ width: W, height: SPHERE_H }}>
      <Animated.View style={{ position: 'absolute', width: 72, height: 72, borderRadius: 36, left: SPHERE_CX - 36, top: SPHERE_CY - 36, backgroundColor: PRIMARY, opacity: glowOpacity, transform: [{ scale: glowScale }], shadowColor: PRIMARY, shadowRadius: 32, shadowOpacity: 1, elevation: 10 }} />
...
function TypingDots() {
  ...
  return (
    <View style={S.dotsRow}>
      {dots.map((d, i) => (
        <Animated.View key={i} style={[S.dot, { opacity: d.interpolate({ inputRange: [0, 1], outputRange: [0.35, 1] }), transform: [{ scale: d.interpolate({ inputRange: [0, 1], outputRange: [1, 1.4] }) }] }]} />
      ))}
    </View>
  );
}
...
function MarketCard({ data }) {
  ...
  return (
    <View style={S.mktCard}>
      ...
      {!!insight && <View style={S.mktTip}><Ionicons name="bulb-outline" size={12} color="#F59E0B" /><Text style={S.mktTipText}>{insight}</Text></View>}
    </View>
  );
}
...
// ── Voice tab view ─────────────────────────────────────────────────────────
function VoiceParticleView({ isRecording, isProcessing, audioLevel, recordDuration, voiceResult, onStart, onSend, onCancel, onViewChat }) {
  const fadeAnim  = useRef(new Animated.Value(0)).current;
      ...
      {voiceResult && !isRecording && !isProcessing && (
        <Animated.View style={[VP.resultCard, { opacity: transFade }]}>
          {voiceResult.error ? (
            ...
            <>
              <View style={VP.resultRow}><Ionicons name="mic-outline" size={13} color={MUTED} /><Text style={VP.resultTrans} numberOfLines={2}>{voiceResult.transcription}</Text></View>
              <View style={[VP.resultRow, { marginTop: 8 }]}><Ionicons name="leaf-outline" size={13} color={PRIMARY} /><Text style={VP.resultReply} numberOfLines={3}>{voiceResult.reply}</Text></View>
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity style={SB.sessionRow} onPress={() => { onSessionPress(item); onClose(); }} activeOpacity={0.75}>
                  <View style={[SB.sessionIcon, { backgroundColor: isScan ? 'rgba(34,197,94,0.12)' : 'rgba(20,184,166,0.12)' }]}>
      ...
      {TABS.map(tab => {
        ...
        return (
          <TouchableOpacity key={tab.id} style={S.tab} onPress={() => onChange(tab.id)} activeOpacity={0.75}>
            ...
            <Ionicons name={on ? tab.icon : `${tab.icon}-outline`} size={18} color={on ? PRIMARY : MUTED} />
            <Text style={[S.tabLabel, on && S.tabLabelActive]}>{tab.label}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
...
export default function AIChatScreen({ navigation, route }) {
  ...
  // Null-safe context access — prevents crash if context not ready
  ...
  const startTab               = route?.params?.voiceMode ? 'voice' : 'chat';
  ...
  // ── History (loaded when sidebar opens) ──────────────────────────────────────
  ...
  return (
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      <LinearGradient colors={['rgba(34,197,94,0.09)', 'transparent']} style={S.orb1} pointerEvents="none" />
      <LinearGradient colors={['rgba(20,184,166,0.07)', 'transparent']} style={S.orb2} pointerEvents="none" />
      ...
      {activeTab === 'chat' && (
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          {/* Compact suggestion chips — only before first message */}
          {messages.length <= 1 && (
          ...
          {/* Input bar — no mic button, just send */}
            ...
            <View style={S.inputRow}>
              ...
              >
                {typing ? (
                  ...
                  <LinearGradient colors={input.trim() ? [USER_A, USER_B] : ['rgba(255,255,255,0.06)', 'rgba(255,255,255,0.06)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={S.sendBtnGrad}>
                    <Ionicons name="arrow-up" size={18} color={input.trim() ? '#FFF' : MUTED} />
                  </LinearGradient>
                )}
              </TouchableOpacity>
            </View>
...
const S = StyleSheet.create({
  ...
  tabBar: { flexDirection: 'row', backgroundColor: 'rgba(7,16,9,0.92)', borderBottomWidth: 1, borderBottomColor: GBORDER },
  tab: { flex: 1, alignItems: 'center', paddingVertical: 10, gap: 3, position: 'relative' },
  tabIndicator: { position: 'absolute', top: 0, left: '15%', right: '15%', height: 2, backgroundColor: PRIMARY, borderRadius: 1 },
  ...
  aiAvatar: { width: 30, height: 30, borderRadius: 9, marginTop: 2, backgroundColor: GLASS, borderWidth: 1, borderColor: GBORDER, justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
  aiBubble: { backgroundColor: GLASS, borderRadius: 18, borderBottomLeftRadius: 4, paddingHorizontal: 14, paddingVertical: 12, maxWidth: W * 0.78, borderWidth: 1, borderColor: GBORDER },
  aiBubbleText: { fontSize: 14, color: TEXT, lineHeight: 21 },

  userBubbleWrap: { alignItems: 'flex-end', marginBottom: 2 },
  userBubble: { borderRadius: 18, borderBottomRightRadius: 4, paddingHorizontal: 16, paddingVertical: 12, maxWidth: W * 0.78 },
  ...
  // ── Chips: compact single-line pills ──────────────────────────────────────
  chipsList: { paddingHorizontal: 14, paddingVertical: 6, gap: 7 },
  chip: {
  ...
  // ── Input bar ──────────────────────────────────────────────────────────────
  inputBar: { backgroundColor: 'rgba(7,16,9,0.95)', paddingHorizontal: 14, paddingTop: 8 },
  inputFade: { position: 'absolute', top: -24, left: 0, right: 0, height: 24 },
  inputRow: {
    flexDirection: 'row', alignItems: 'flex-end', gap: 8,
    backgroundColor: GLASS, borderRadius: 24, borderWidth: 1, borderColor: GBORDER,
    paddingHorizontal: 14, paddingVertical: 8,
  },
  textInput: { flex: 1, fontSize: 14, color: TEXT, maxHeight: 100, minHeight: 36, paddingVertical: 4 },
  ...
  // ── Diagnosis / Market cards ───────────────────────────────────────────────
  diagCard: { backgroundColor: GLASS, borderRadius: 16, padding: 14, gap: 10, borderWidth: 1, borderColor: GBORDER, maxWidth: W * 0.78 },
  ...
  diagStepNum: { width: 20, height: 20, borderRadius: 10, backgroundColor: 'rgba(34,197,94,0.15)', justifyContent: 'center', alignItems: 'center', flexShrink: 0, marginTop: 1 },
  ...
  mktCard: { backgroundColor: GLASS, borderRadius: 16, padding: 14, gap: 8, borderWidth: 1, borderColor: GBORDER, maxWidth: W * 0.78 },
...
const VP = StyleSheet.create({
  ...
  statusPill: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: GLASS, borderRadius: 20, paddingHorizontal: 16, paddingVertical: 8, borderWidth: 1, borderColor: GBORDER },
  statusPillActive: { borderColor: PRIMARY },
  statusDot: { width: 8, height: 8, borderRadius: 4 },
  statusLabel: { fontSize: 14, fontWeight: '700', color: TEXT },
  resultCard: { backgroundColor: GLASS, borderRadius: 20, padding: 16, gap: 4, borderWidth: 1, borderColor: GBORDER, maxWidth: W - 48 },
...
const SB = StyleSheet.create({
  ...
  panel: {
    ...
    borderRightWidth: 1, borderRightColor: GBORDER,
  ...
  closeBtn: { width: 32, height: 32, borderRadius: 16, justifyContent: 'center', alignItems: 'center', backgroundColor: GLASS },
  ...
  sessionRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: 'rgba(34,197,94,0.08)' },
