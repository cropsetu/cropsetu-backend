...
const PRIORITY = {
  urgent: { color: '#E74C3C', bg: '#2B0D0D', tKey: 'urgent' },
  today:  { color: '#F39C12', bg: '#2B1D00', tKey: 'today'  },
  plan:   { color: '#2ECC71', bg: '#0D2B18', tKey: 'planned'},
};
...
const ALL_TASKS = [
  {
    ...
    time: 'Before 10 AM',
    icon: 'flask-outline', color: '#E74C3C',
  ...
  },
  {
    ...
    time: 'This week',
    icon: 'earth-outline', color: '#E67E22',
  ...
  },
  {
    ...
    crop: 'Tomato', field: '',
    time: 'Thursday',
    icon: 'storefront-outline', color: '#9B59B6',
...
function TaskItem({ task, onToggle, t }) {
  ...
  return (
    <View style={[T.item, { backgroundColor: task.doneAt ? '#0D0D0D' : p.bg, borderColor: task.doneAt ? 'rgba(255,255,255,0.04)' : `${p.color}25` }]}>
      <TouchableOpacity style={T.itemTop} onPress={toggle} activeOpacity={0.8}>
        <TouchableOpacity style={[T.checkbox, task.doneAt && { backgroundColor: '#2ECC71', borderColor: '#2ECC71' }]} onPress={() => onToggle(task.id)}>
          {task.doneAt && <Ionicons name="checkmark" size={14} color="#0A140A" />}
        </TouchableOpacity>
        <View style={[T.taskIcon, { backgroundColor: `${task.color}18` }]}>
          <Ionicons name={task.icon} size={16} color={task.doneAt ? '#555' : task.color} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={[T.taskTitle, task.doneAt && T.taskDone]}>{task.title}</Text>
          <View style={T.taskMeta}>
            <Ionicons name="leaf-outline" size={10} color="#888" />
      ...
      {open && (
        <View style={T.expanded}>
          ...
          <View style={T.aiReason}>
            <Ionicons name="hardware-chip-outline" size={12} color="#2ECC71" />
      ...
      <View style={[T.header, { paddingTop: insets.top + 10 }]}>
        ...
        <TouchableOpacity style={T.aiGenBtn} onPress={() => handleGenerate(true)} activeOpacity={0.8} disabled={generating}>
          {generating
            ...
            : <Ionicons name="sparkles-outline" size={14} color="#F39C12" />}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={T.list}>
        {loading ? (
          <View style={{ alignItems: 'center', paddingVertical: 48 }}>
            ...
            <Text style={{ color: '#666', marginTop: 12, fontSize: 13 }}>{t('planner.generatingAI')}</Text>
          </View>
        ) : tasks.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            <Ionicons name="checkmark-done-circle-outline" size={48} color="#333" />
            <Text style={{ color: '#666', marginTop: 10, fontSize: 14 }}>{t('planner.noTasks')}</Text>
            <TouchableOpacity onPress={() => handleGenerate(true)} style={{ marginTop: 14, backgroundColor: 'rgba(46,204,113,0.12)', borderRadius: 12, paddingHorizontal: 20, paddingVertical: 10 }}>
              <Text style={{ color: '#2ECC71', fontWeight: '700' }}>{t('planner.generateWithAI')}</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
            {renderSection(t('planner.sectionUrgent'), urgent, '#E74C3C')}
            {renderSection(t('planner.sectionToday'), today_, '#F39C12')}
            {renderSection(t('planner.sectionPlanned'), planned, '#2ECC71')}
            {doneTasks.length > 0 && renderSection(t('planner.sectionDone'), doneTasks, '#555')}
          </>
        )}
      </ScrollView>
    </View>
...
const T = StyleSheet.create({
  ...
  aiGenText: { fontSize: 11, fontWeight: '700', color: '#F39C12' },
  ...
  progressPct:  { fontSize: 16, fontWeight: '900', color: '#2ECC71' },
  ...
  allDoneText:  { fontSize: 12, color: '#2ECC71', fontWeight: '600' },
  ...
  sectionTitle:{ fontSize: 11, fontWeight: '900', color: '#9CA3AF', letterSpacing: 1.2, textTransform: 'uppercase', flex: 1 },
  ...
  expandedDetail: { fontSize: 13, color: '#6B7280', lineHeight: 19 },
  aiReason: {
    flexDirection: 'row', alignItems: 'flex-start', gap: 7,
    backgroundColor: 'rgba(46,204,113,0.08)',
    borderRadius: 8, padding: 10,
  },
  aiReasonText: { flex: 1, fontSize: 11, color: '#6B7280', lineHeight: 16 },
});
