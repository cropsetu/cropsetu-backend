...
const SYMPTOM_KEYS = [
  { key: 'yellow_leaves', tKey: 'sym_yellow_leaves', icon: 'leaf-outline'          },
  { key: 'brown_spots',   tKey: 'sym_brown_spots',   icon: 'ellipse-outline'       },
  { key: 'white_powder',  tKey: 'sym_white_powder',  icon: 'snow-outline'          },
  { key: 'wilting',       tKey: 'sym_wilting',       icon: 'trending-down-outline' },
  { key: 'insects',       tKey: 'sym_insects',       icon: 'bug-outline'           },
  { key: 'holes',         tKey: 'sym_holes',         icon: 'aperture-outline'      },
  { key: 'stunted',       tKey: 'sym_stunted',       icon: 'resize-outline'        },
  { key: 'fruit_damage',  tKey: 'sym_fruit_damage',  icon: 'nutrition-outline'     },
  { key: 'stem_rot',      tKey: 'sym_stem_rot',      icon: 'git-merge-outline'     },
  { key: 'curling_leaves',tKey: 'sym_curling_leaves',icon: 'refresh-outline'       },
  { key: 'root_rot',      tKey: 'sym_root_rot',      icon: 'git-network-outline'   },
  { key: 'pale_color',    tKey: 'sym_pale_color',    icon: 'contrast-outline'      },
];
  ...
  return (
    <View style={[SC.stepDot,
      ...
      {done
        ...
        : <Text style={[SC.stepDotNum, active && { color: '#FFFFFF' }]}>{step}</Text>
      }
    </View>
  );
}
...
function GradientBtn({ onPress, disabled, colors = ['#16A34A', '#22C55E'], style, children }) {
  const scale = useRef(new Animated.Value(1)).current;
...
export default function CropScanScreen({ navigation }) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        try {
          navigation.replace('DiagnosisResult', { diagnosis, farmContext: farmCtx });
        } catch (navErr) {
    ...
    <View style={SC.root}>
      ...
      <View style={[SC.header, { paddingTop: insets.top + 8 }]}>
        ...
        <View style={SC.aiBadge}>
          <Ionicons name="hardware-chip" size={11} color="#2ECC71" />
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={SC.scrollContent} showsVerticalScrollIndicator={false}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={SC.chipRow}>
                {COMMON_CROPS.map(c => {
                  const active = !showCustomCrop && selectedCrop === c;
                  return (
                    <AnimChip
                      key={c}
                      chipStyle={[SC.chip, active && SC.chipActive]}
                      onPress={() => { setSelectedCrop(c); setShowCustomCrop(false); }}
                    >
                      <Text style={[SC.chipText, active && SC.chipTextActive]}>{c}</Text>
                    </AnimChip>
                  );
                })}
                <AnimChip
                ...
                >
                  <Ionicons name="add" size={14} color={showCustomCrop ? '#FFFFFF' : '#6B7280'} />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={SC.chipRow}>
                {SOIL_TYPES.map(s => (
                  <AnimChip
                    key={s.key}
                    chipStyle={[SC.chip, soilType === s.key && SC.chipActive]}
                    onPress={() => setSoilType(soilType === s.key ? '' : s.key)}
                  >
                    <Text style={[SC.chipText, soilType === s.key && SC.chipTextActive]}>{s.label}</Text>
                  </AnimChip>
                ))}
              </ScrollView>
              </AnimCard>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={SC.chipRow}>
                {IRRIGATION_TYPES.map(ir => (
                  <AnimChip
                    key={ir.key}
                    chipStyle={[SC.chip, irrigation === ir.key && SC.chipActive]}
                    onPress={() => setIrrigation(irrigation === ir.key ? '' : ir.key)}
                  >
                    <Text style={[SC.chipText, irrigation === ir.key && SC.chipTextActive]}>{ir.label}</Text>
                  </AnimChip>
                ))}
              </ScrollView>
              </AnimCard>
              ...
              {(farmProfile.location?.state || farmProfile.landSize) && (
                <View style={SC.profileHint}>
                  <Ionicons name="information-circle-outline" size={13} color="#3498DB" />
              ...
              <View style={SC.symptomGrid}>
                {SYMPTOM_KEYS.map(sym => {
                  ...
                  return (
                    ...
                    >
                      <Ionicons name={sym.icon} size={16} color={active ? '#FFFFFF' : '#6B7280'} />
              ...
              <View style={SC.areaRow}>
                {AREA_KEYS.map(o => (
                  ...
                  >
                    ...
                    <Text style={[SC.areaBtnDesc, affectedArea === o.key && { color: '#16A34A' }]}>
                      {t(`cropScan.${o.tDesc}`)}
                    </Text>
                  </AnimChip>
                ))}
          ...
          <ScrollView contentContainerStyle={SC.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={SC.summaryCard}>
              ...
              <View style={SC.summaryRow}>
                <Ionicons name="leaf" size={13} color="#2ECC71" />
                <Text style={SC.summaryText}>{t('cropScan.cropLabel')} <Text style={{ color: '#1E293B', fontWeight: '700' }}>{cropName || '—'}</Text></Text>
              </View>
              {cropAge ? (
                <View style={SC.summaryRow}>
                  <Ionicons name="time" size={13} color="#2ECC71" />
                  <Text style={SC.summaryText}>{t('cropScan.ageLabel')} <Text style={{ color: '#1E293B', fontWeight: '700' }}>{cropAge} {t('cropScan.daysUnit')}</Text></Text>
                </View>
              ) : null}
              {selectedSymptoms.size > 0 && (
                <View style={SC.summaryRow}>
                  <Ionicons name="alert-circle" size={13} color="#F39C12" />
                  <Text style={SC.summaryText} numberOfLines={2}>
                    {t('cropScan.symptomsLabel')} <Text style={{ color: '#1E293B', fontWeight: '700' }}>
              ...
              {(soilType || farmProfile.soilType) && (
                <View style={SC.summaryRow}>
                  <Ionicons name="layers" size={13} color="#E67E22" />
                  <Text style={SC.summaryText}>
                    {t('cropScan.soilLabel')} <Text style={{ color: '#1E293B', fontWeight: '700' }}>
                      {SOIL_TYPES.find(s => s.key === (soilType || farmProfile.soilType))?.label || soilType}
                    </Text>
                  </Text>
                </View>
              )}
                ...
                <View style={SC.contextBadges}>
                  {cropName ? (
                    <View style={SC.contextBadge}>
                      <Ionicons name="leaf" size={11} color="#2ECC71" />
                      <Text style={SC.contextBadgeText}>{cropName}</Text>
                    </View>
                  ) : null}
                  {cropAge ? (
                    <View style={SC.contextBadge}>
                      <Ionicons name="time" size={11} color="#2ECC71" />
                      <Text style={SC.contextBadgeText}>{cropAge} days</Text>
                    </View>
                  ) : null}
                  {selectedSymptoms.size > 0 && (
                    <View style={SC.contextBadge}>
                      <Ionicons name="alert-circle" size={11} color="#F39C12" />
...
const SC = StyleSheet.create({
  ...
  aiBadgeText: { fontSize: 10, fontWeight: '800', color: '#16A34A' },
  ...
  stepDotActive:   { backgroundColor: '#16A34A', borderColor: '#16A34A' },
  stepDotDone:     { backgroundColor: '#16A34A', borderColor: '#16A34A' },
  stepDotNum:      { fontSize: 11, fontWeight: '800', color: '#6B7280' },
  ...
  inputUnit:    { fontSize: 13, color: '#64748B', marginBottom: 4, width: 40 },
  ...
  profileHintText: { fontSize: 11, color: '#3498DB', flex: 1 },
  ...
  symptomChipActive:     { backgroundColor: '#16A34A', borderColor: '#16A34A' },
  symptomChipText:       { fontSize: 12, color: '#374151', fontWeight: '600', flex: 1 },
  symptomChipTextActive: { color: '#FFFFFF' },
  ...
  optionBtnActive:    { backgroundColor: 'rgba(22,163,74,0.1)', borderColor: '#16A34A' },
  optionBtnText:      { fontSize: 12, color: '#374151', fontWeight: '600' },
  optionBtnTextActive:{ color: '#16A34A', fontWeight: '700' },
  ...
  areaBtnActive:    { backgroundColor: 'rgba(22,163,74,0.1)', borderColor: '#16A34A' },
  areaBtnPct:       { fontSize: 14, fontWeight: '800', color: '#374151' },
  areaBtnPctActive: { color: '#16A34A' },
  areaBtnDesc:      { fontSize: 10, color: '#64748B', marginTop: 2 },
  ...
  photoTipTitle: { fontSize: 12, fontWeight: '800', color: '#F39C12', marginBottom: 4 },
  photoTipText:  { fontSize: 11, color: '#374151', lineHeight: 17 },
  ...
  photoPickerTitle: { fontSize: 15, fontWeight: '800', color: '#1E293B' },
  photoPickerSub:   { fontSize: 12, color: '#64748B' },
  ...
  previewBadgeText: { fontSize: 11, color: '#2ECC71', fontWeight: '700' },
  changePhotoBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
  ...
  changePhotoBtnText: { fontSize: 12, color: '#F39C12', fontWeight: '700' },
  ...
  summaryTitle: { fontSize: 11, fontWeight: '800', color: '#64748B', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 },
  summaryRow:   { flexDirection: 'row', alignItems: 'flex-start', gap: 8 },
  summaryText:  { fontSize: 12, color: '#374151', flex: 1 },
  ...
  nextBtnGradient: {
    ...
    shadowColor: '#16A34A', shadowOpacity: 0.35, shadowRadius: 10, shadowOffset: { width: 0, height: 4 },
    elevation: 5,
  },
  nextBtnDisabled: { backgroundColor: '#D1D5DB' },
  analyseBtn:      { backgroundColor: '#16A34A' },
  ...
  analysisMainText: { fontSize: 20, fontWeight: '900', color: '#1E293B', textAlign: 'center' },
  analysisSubText:  { fontSize: 12, color: '#64748B', textAlign: 'center' },
  ...
  contextBadgeText: { fontSize: 11, color: '#374151', fontWeight: '600' },
  ...
  progressDotDone:   { backgroundColor: '#16A34A', borderColor: '#16A34A' },
  progressDotActive: { backgroundColor: '#F59E0B', borderColor: '#F59E0B' },
  progressText:      { fontSize: 13, color: '#64748B', flex: 1 },
  progressTextDone:  { color: '#16A34A' },
  progressTextActive:{ color: '#1E293B', fontWeight: '700' },
  analysisNote: { fontSize: 11, color: '#64748B', textAlign: 'center', fontStyle: 'italic' },

  // Error
  errorBox: { alignItems: 'center', gap: 12, paddingHorizontal: 20 },
  errorTitle: { fontSize: 18, fontWeight: '900', color: '#E74C3C' },
  errorMsg:   { fontSize: 13, color: '#6B7280', textAlign: 'center', lineHeight: 20 },
  retryBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#16A34A', borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12,
    marginTop: 8,
  },
