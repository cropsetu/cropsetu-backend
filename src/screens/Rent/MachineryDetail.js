    ...
    <View>
      ...
      <View style={C.calGrid}>
        {cells.map((day, i) => {
          ...
          let txtColor = past ? '#ccc' : '#1A1A1A';
          if (booked)           { bgColor = '#FFCDD2'; txtColor = '#C62828'; }
          if (inRange && !booked) bgColor = GREEN + '25';
          if (isStart || isEnd)  bgColor = GREEN;
          if (isStart || isEnd)  txtColor = '#fff';

          return (
            <TouchableOpacity
              key={dk}
              style={[C.calCell, { backgroundColor: bgColor, borderRadius: 8 },
                today && !isStart && !isEnd && { borderWidth: 1.5, borderColor: GREEN }]}
      ...
      <View style={C.legend}>
        <View style={C.legendItem}>
          <View style={[C.legendDot, { backgroundColor: '#FFCDD2' }]} />
          <Text style={C.legendTxt}>{t('rent.occupied')}</Text>
        </View>
        <View style={C.legendItem}>
          <View style={[C.legendDot, { backgroundColor: GREEN }]} />
          <Text style={C.legendTxt}>{t('rent.yourSelection')}</Text>
        </View>
        <View style={C.legendItem}>
          <View style={[C.legendDot, { backgroundColor: '#fff', borderWidth: 1.5, borderColor: GREEN }]} />
...
// ── Spec row ──────────────────────────────────────────────────────────────────
function SpecRow({ icon, label, value, color = '#444' }) {
        ...
        <View style={D.galleryWrap}>
          {allMedia.length > 0 ? (
            ...
            <View style={[D.galleryImgFallback]}>
              <Ionicons name="construct" size={80} color="#1565C0" />
            </View>
          )}
          ...
          <View style={[D.availOverlay, { backgroundColor: m.available ? '#E8F5E9' : '#FFF3E0' }]}>
            <View style={[D.availDot, { backgroundColor: m.available ? GREEN : '#E65100' }]} />
            <Text style={[D.availTxt, { color: m.available ? GREEN : '#E65100' }]}>
        ...
        <View style={D.content}>
          ...
          <View style={D.titleRow}>
            <View style={{ flex: 1 }}>
              ...
              {m.brand && <Text style={D.subtitle}>{m.brand}</Text>}
            </View>
            <View style={D.priceBox}>
              {m.pricePerHour && <Text style={D.priceHr}>₹{m.pricePerHour?.toLocaleString()}/hr</Text>}
              <Text style={D.priceDay}>₹{m.pricePerDay?.toLocaleString()}/day</Text>
              {m.pricePerAcre && <Text style={D.priceAcre}>₹{m.pricePerAcre?.toLocaleString()}/acre</Text>}
            </View>
          </View>
          ...
          {m.rating > 0 && (
            <View style={D.ratingRow}>
              {[1,2,3,4,5].map(s => (
                <Ionicons key={s} name={s <= Math.round(m.rating) ? 'star' : 'star-outline'} size={15} color="#F9A825" />
              ))}
          ...
          <View style={D.specsCard}>
            <SpecRow icon="calendar-outline"   label={t('rent.ageLabel')}        value={m.ageYears     ? `${m.ageYears} yr${m.ageYears > 1 ? 's' : ''}` : null} color="#1565C0" />
            <SpecRow icon="speedometer-outline" label={t('rent.usageLabel')}      value={m.mileageHours ? `${m.mileageHours.toLocaleString()} h` : null}            color="#6A1B9A" />
            <SpecRow icon="flash-outline"       label={t('rent.powerLabel')}      value={m.horsePower}  color="#E65100" />
            <SpecRow icon="flame-outline"       label={t('rent.fuelLabel')}       value={m.fuelType ? t('rent.fuel' + m.fuelType.charAt(0).toUpperCase() + m.fuelType.slice(1)) : null} color="#C62828" />
            <SpecRow icon="location-outline"    label={t('rent.locationLabel')}   value={m.location}    color="#2E7D32" />
            {m.availableFrom && (
            ...
            <View style={D.selCard}>
              ...
              {selEnd && (
                <>
                  ...
                  <View style={[D.selRow, { borderTopWidth: 1, borderTopColor: '#eee', marginTop: 8, paddingTop: 8 }]}>
...
const D = StyleSheet.create({
  ...
  galleryImgFallback: { width: W, height: 300, backgroundColor: '#E3F2FD', justifyContent: 'center', alignItems: 'center' },
  ...
  content: { padding: 16, backgroundColor: '#F8F9FA', marginTop: -16, borderTopLeftRadius: 20, borderTopRightRadius: 20 },

  titleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 },
  title:    { fontSize: 20, fontWeight: '800', color: '#1A1A1A', flex: 1, marginRight: 8 },
  subtitle: { fontSize: 13, color: '#888', marginTop: 2 },
  priceBox: { alignItems: 'flex-end' },
  priceHr:  { fontSize: 13, color: '#888' },
  priceDay: { fontSize: 18, fontWeight: '900', color: '#2D9162' },
  priceAcre:{ fontSize: 11, color: '#888', marginTop: 1 },

  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: 3, marginBottom: 16 },
  ratingTxt: { fontSize: 13, color: '#888', marginLeft: 4 },

  sectionTitle: { fontSize: 16, fontWeight: '800', color: '#1A1A1A', marginBottom: 10, marginTop: 6 },

  specsCard: { backgroundColor: '#fff', borderRadius: 18, padding: 4, marginBottom: 16, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 12, shadowOffset: { width: 0, height: 3 }, elevation: 4 },
  ...
  specLabel: { fontSize: 11, color: '#888', fontWeight: '600' },
  specValue: { fontSize: 14, color: '#1A1A1A', fontWeight: '700', marginTop: 1 },

  featureWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 },
  featureChip: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#E8F5EE', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 7 },
  featureTxt:  { fontSize: 12, color: '#2D9162', fontWeight: '700' },

  descTxt: { fontSize: 14, color: '#555', lineHeight: 22, marginBottom: 16 },

  calCard:     { backgroundColor: '#fff', borderRadius: 16, padding: 14, marginBottom: 16, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 6, elevation: 2 },
  calHeader:   { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 },
  calNavBtn:   { padding: 8 },
  calMonthTxt: { fontSize: 15, fontWeight: '800', color: '#1A1A1A' },

  selCard: { backgroundColor: '#E8F5EE', borderRadius: 16, padding: 14, marginBottom: 16, borderWidth: 1.5, borderColor: '#2D9162' + '40' },
  selTitle: { fontSize: 14, fontWeight: '800', color: '#2D9162', marginBottom: 10 },
  selRow:   { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 },
  selTxt:   { fontSize: 14, color: '#1A1A1A', fontWeight: '600', flex: 1 },
  totalLabel:{ fontSize: 14, color: '#1A1A1A', fontWeight: '700', flex: 1 },
  totalAmt: { fontSize: 18, fontWeight: '900', color: '#2D9162' },
  notesInput: { marginTop: 10, backgroundColor: '#fff', borderRadius: 10, borderWidth: 1.5, borderColor: '#ddd', padding: 10, fontSize: 13, color: '#1A1A1A', minHeight: 50 },

  ownerCard:    { backgroundColor: '#fff', borderRadius: 16, padding: 14, marginBottom: 16, flexDirection: 'row', alignItems: 'center', gap: 12, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  ownerAvatar:  { width: 46, height: 46, borderRadius: 23, backgroundColor: '#2D9162', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
  ownerAvatarImg:{ width: 46, height: 46, borderRadius: 23 },
  ownerName:    { fontSize: 15, fontWeight: '700', color: '#1A1A1A' },
  ownerLoc:     { fontSize: 12, color: '#888', marginTop: 2 },
  callSmall:    { width: 40, height: 40, borderRadius: 20, borderWidth: 2, borderColor: '#2D9162', justifyContent: 'center', alignItems: 'center' },

  bottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', gap: 10, padding: 12, paddingTop: 10, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#eee' },
  callBtn:   { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 2, borderColor: '#2D9162', borderRadius: 14, paddingVertical: 13 },
  callBtnTxt:{ fontSize: 14, fontWeight: '700', color: '#2D9162' },
  bookBtn2:  { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, backgroundColor: '#2D9162', borderRadius: 14, paddingVertical: 13 },
  bookBtn2Txt:{ fontSize: 13, fontWeight: '800', color: '#fff' },
});

const C = StyleSheet.create({
  calWeekRow: { flexDirection: 'row', marginBottom: 4 },
  calDayName: { flex: 1, textAlign: 'center', fontSize: 10, fontWeight: '700', color: '#999' },
  ...
  calBookedDot:{ fontSize: 5, color: '#C62828', marginTop: -2 },
  legend:     { flexDirection: 'row', justifyContent: 'center', gap: 16, marginTop: 12 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  legendDot:  { width: 12, height: 12, borderRadius: 4 },
  legendTxt:  { fontSize: 10, color: '#666', fontWeight: '600' },
});
