...
import { CROP_IMAGES, SOIL_IMAGES, IRRIGATION_IMAGES } from '../constants/cropImages';
...
const GREEN   = '#1A5C2A';
const GREEN6  = 'rgba(26,92,42,0.06)';
const GREEN20 = 'rgba(26,92,42,0.20)';
    ...
    <View style={[S.wrapper, style]}>
      ...
      >
        ...
        {cropName ? (
          ...
          <Chip empty>
            <Ionicons name="leaf-outline" size={15} color="#9CA3AF" />
        ...
        {soilKey ? (
          <Chip>
            {SOIL_IMAGES[soilKey] ? (
              <Image
                source={SOIL_IMAGES[soilKey]}
                style={[S.thumbImg, { width: thumbSize, height: thumbSize }]}
              />
            ) : (
              <View style={[S.thumbPlaceholder, { width: thumbSize, height: thumbSize }]}>
                <Ionicons name="layers-outline" size={12} color={GREEN} />
              </View>
            )}
            <Text style={[S.chipLabel, { fontSize: labelSize }]} numberOfLines={1}>
        ...
        {irrigationKey ? (
          <Chip>
            {IRRIGATION_IMAGES[irrigationKey] ? (
              <Image
                source={IRRIGATION_IMAGES[irrigationKey]}
                style={[S.thumbImg, { width: thumbSize, height: thumbSize }]}
              />
            ) : (
              <View style={[S.thumbPlaceholder, { width: thumbSize, height: thumbSize }]}>
                <Ionicons name="water-outline" size={12} color="#0288D1" />
              </View>
            )}
            <Text style={[S.chipLabel, { fontSize: labelSize }]} numberOfLines={1}>
        ...
        {landSize ? (
          <Chip>
            <Ionicons name="resize-outline" size={13} color="#6D4C41" />
            <Text style={[S.chipLabel, { fontSize: labelSize }]} numberOfLines={1}>
              {landSize} acres
            </Text>
          </Chip>
...
const S = StyleSheet.create({
  ...
  iconWrap: {
    borderRadius: 999,
    overflow: 'hidden',
    backgroundColor: 'rgba(26,92,42,0.06)',
